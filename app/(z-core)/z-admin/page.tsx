        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="bg-background border border-border rounded-xl pl-10 pr-4 py-2 text-sm w-64 focus:outline-none focus:border-accent"
            />
          </div>
          <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center font-bold text-black cursor-pointer">
            A
          </div>
        </div>
      </header>
          ...
          {isLoading ? (
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[
                  { label: 'Total Users', value: stats?.userCount || 0, change: '0%', color: 'text-blue-500' },
                  { label: 'Active Today', value: 0, change: '0%', color: 'text-green-500' },
                  { label: 'Pending Reports', value: stats?.reportCount || 0, change: '0%', color: 'text-orange-500' },
                  { label: 'Trust Index', value: '100%', change: '0%', color: 'text-accent' },
                ].map((stat) => (
                  <div key={stat.label} className="bg-muted border border-border p-6 rounded-3xl">
                    ...
                    <div className="text-[10px] font-bold mt-2 text-foreground/30">{stat.change} from last week</div>
                  </div>
                ))}
              </div>
