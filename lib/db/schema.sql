...
CREATE TABLE public.contact_submissions (
  ...
  name text,
  school_name text,
  status text NOT NULL DEFAULT 'new'::text CHECK (status = ANY (ARRAY['new'::text, 'reviewing'::text, 'resolved'::text, 'closed'::text, 'spam'::text])),
  source text NOT NULL DEFAULT 'contact_page'::text,
  user_agent text,
  ip_address text,
...
CREATE TABLE public.feedback_submissions (
  ...
  message text NOT NULL,
  school_name text,
  status text NOT NULL DEFAULT 'new'::text CHECK (status = ANY (ARRAY['new'::text, 'reviewing'::text, 'planned'::text, 'resolved'::text, 'closed'::text, 'spam'::text])),
  source text NOT NULL DEFAULT 'feedback_page'::text,
  user_agent text,
  ip_address text,
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.z_ai_usage_limits (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  scope text NOT NULL DEFAULT 'default'::text UNIQUE,
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT z_ai_usage_limits_pkey PRIMARY KEY (id)
);
CREATE TABLE public.z_ai_conversations (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  persona_id uuid,
  mode text NOT NULL DEFAULT 'chat'::text CHECK (mode = ANY (ARRAY['chat'::text, 'call'::text])),
  status text NOT NULL DEFAULT 'active'::text CHECK (status = ANY (ARRAY['active'::text, 'ended'::text, 'rate_limited'::text, 'failed'::text])),
  title text,
  ...
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  CONSTRAINT z_ai_conversations_pkey PRIMARY KEY (id),
  CONSTRAINT z_ai_conversations_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT z_ai_conversations_persona_id_fkey FOREIGN KEY (persona_id) REFERENCES public.personas(id)
);
CREATE TABLE public.z_ai_messages (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  conversation_id uuid NOT NULL,
  user_id uuid NOT NULL,
  persona_id uuid,
  role text NOT NULL CHECK (role = ANY (ARRAY['user'::text, 'assistant'::text, 'system'::text])),
  input_type text NOT NULL DEFAULT 'text'::text CHECK (input_type = ANY (ARRAY['text'::text, 'speech'::text, 'system'::text])),
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  CONSTRAINT z_ai_messages_pkey PRIMARY KEY (id),
  CONSTRAINT z_ai_messages_conversation_id_fkey FOREIGN KEY (conversation_id) REFERENCES public.z_ai_conversations(id),
  CONSTRAINT z_ai_messages_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT z_ai_messages_persona_id_fkey FOREIGN KEY (persona_id) REFERENCES public.personas(id)
);
CREATE TABLE public.z_ai_usage_events (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  conversation_id uuid,
  event_type text NOT NULL CHECK (event_type = ANY (ARRAY['chat_message'::text, 'voice_input'::text, 'voice_output'::text, 'call_started'::text, 'call_ended'::text, 'tts'::text])),
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  CONSTRAINT z_ai_usage_events_pkey PRIMARY KEY (id),
  CONSTRAINT z_ai_usage_events_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT z_ai_usage_events_conversation_id_fkey FOREIGN KEY (conversation_id) REFERENCES public.z_ai_conversations(id)
);